# Здравствуйте, Роман! Ввиду близости жесткого дедлайна и учитывая
# рекомендательный характер пунктов 3 и 6, а так же учитывая объем изменений, требуемых
# для их реализации, в данной версии работы нет соответствующих этим пунктам доработок.
#
# Дайте знать, если их всё-таки необходимо сделать для сдачи. Спасибо!
#
# Остальные пункты вашего замечания выполнены.
# Плюс убраны данные о пароле и пользователе из файла конфига и перенесены
# в .env файл. Сам файл не трекается git'ом, но вы можете посмотреть его пример в
# .env_example
#
# Как я уже знаю из вебинара, данный скрипт еще пригодится в будущем, так что доработки
# я всё-таки планирую ввести, в том числе перевести процесс в docker.
#
# Предложенный вами алгоритм в пункте 3 выглядит действительно более оптимальным. То, что я делал
# изначально было ориентированно, в том числе на возможность разнесения функций по разным
# процессам, т.к. об этом упоминалось на вебинаре.
#
